...
class DiskAnalyzer(QObject):
    ...
    def _reset_progress(self, directory_path):
        try:
            self._progress_total = max(1, self._count_items(directory_path))
        except Exception:
    ...
    def _increment_progress(self, step=1):
        self._progress_processed += step
        if self._progress_total <= 0:
            self._progress_total = 1
        percentage = min(99, int((self._progress_processed / self._progress_total) * 100))
        self.progress_updated.emit(percentage)

    def _count_items(self, directory_path):
        total = 0
        try:
            for _root, dirnames, filenames in os.walk(directory_path):
